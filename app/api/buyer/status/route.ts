...
export async function GET(request: NextRequest) {
  ...
  invitationData = invitation.success && (invitation.data ?? []).length > 0 ? (invitation.data as any[])[0] : null
    ...
    // If buyer, get consented vendor count and accepted invoice count
    let vendorCount = null
    let invoiceCount = null
    if ((userRole || "").toLowerCase() === "buyer") {
      // Consented vendors
      const vendorRes = await executeQuery(
        `SELECT COUNT(*) as count FROM vendor_consents WHERE buyer_id = ? AND consent_status = 'consented'`,
        [userId]
      )
      vendorCount = vendorRes.success && (vendorRes.data ?? []).length > 0 ? (vendorRes.data as any[])[0].count : 0

      // Accepted invoices
      const invoiceRes = await executeQuery(
        `SELECT COUNT(*) as count FROM ap_batch_rows WHERE buyer_id = ? AND status = 'accepted'`,
        [userId]
      )
      invoiceCount = invoiceRes.success && (invoiceRes.data ?? []).length > 0 ? (invoiceRes.data as any[])[0].count : 0
    }
